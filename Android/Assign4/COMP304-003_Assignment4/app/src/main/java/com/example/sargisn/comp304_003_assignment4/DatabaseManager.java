...
public class DatabaseManager extends SQLiteOpenHelper {
    //
    private static final String DATABASE_NAME = "RecruitmentAgency.db";
    ...
    public String getCell(String value, String colummName, String tableName, String find) {
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor cursor = null;
        String cell = "";

        cursor = db.rawQuery("SELECT " + colummName + " FROM " +tableName+ " WHERE " + find + " =?", new String[] {value + ""});
        if(cursor.getCount() > 0) {
            cursor.moveToFirst();
            cell = cursor.getString(cursor.getColumnIndex(colummName));
            return cell;
        }
        return "error";
    }

    public boolean getLogin(String username, String password, String tableName)
    {
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor mCursor = db.rawQuery("SELECT * FROM " + tableName + " WHERE userName=? AND password=?", new String[]{username,password});
        if (mCursor != null) {
